...
import JdbcOption.domain.Application_information;
...
public class DealApply extends HttpServlet {
    ...
    protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            String sql="select num from reply where receive_id=? and message=?";
        ...
        try {
            String sql="select time_reply,send_id from reply where receive_id=? and message=?";
            connection= Druid_Utils.getConnection();
            preparedStatement=connection.prepareStatement(sql);
            preparedStatement.setString(1,receive_id);
            preparedStatement.setString(2,"申请加入队伍");
       ...
       List<Application_information> application_list=new ArrayList<>();
        for(String id:applications.keySet()) {
            try {
                ...
                String sql = "select Student_Name,Tel,Strong_point,Major from student_info where Student_ID=?";
                connection = Druid_Utils.getConnection();
                preparedStatement = connection.prepareStatement(sql);
