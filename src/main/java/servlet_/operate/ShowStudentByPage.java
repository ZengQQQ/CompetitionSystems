...
import java.util.Map;
    ...
    public class ShowStudentByPage extends HttpServlet {
        protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
            ...
            // 如果查询结果为空，返回提示信息，但是重定向目标有问题，不跳转
            // 这里修改为跳转到管理员界面
            if(pb==null){
                response.getWriter().print("<script>alert('查询失败，没有匹配的信息!');" +
                                               "window.location.href='http://localhost:8080/_war_exploded/home_page(manager).jsp';</script>");
               return;
            }
            ...
            //验证操作者是不是管理员
            try {
                String sql="select num from manager where id=?";
