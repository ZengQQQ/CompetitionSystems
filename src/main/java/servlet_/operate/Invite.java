...
public class Invite extends HttpServlet {
    ...
    protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try{
            String sql_0="select * from group_table where TID=? and UID=? and state=1";
            connection=Druid_Utils.getConnection();
        ...
        try{
            String sql_0="select * from reply where send_id=? and receive_id=? and message=?";
            connection=Druid_Utils.getConnection();
            preparedStatement=connection.prepareStatement(sql_0);
        ...
        try{
          String sql_0="select * from group_table where TID=? and UID=?";
          connection=Druid_Utils.getConnection();
          preparedStatement=connection.prepareStatement(sql_0);
          preparedStatement.setString(1,id_leader);
          preparedStatement.setString(2,id_student);
        ...
        try{
            String sql_2="select Exist_Num from team_table where Student_ID_leader=?";
            connection=Druid_Utils.getConnection();
        ...
        //校验对方是否被封禁
        try{
            String sql_2="select state from students where Student_ID=?";
            connection=Druid_Utils.getConnection();
            preparedStatement=connection.prepareStatement(sql_2);
            preparedStatement.setString(1,id_student);
            resultSet=preparedStatement.executeQuery();
        ...
        try{
             String sql_1="insert into group_table (TID,UID) values(?,?) ";
            connection=Druid_Utils.getConnection();
        ...
        try{
            String sql_1="update team_table set Exist_Num=Exist_Num-1 where Student_ID_leader=?";
            connection=Druid_Utils.getConnection();
            preparedStatement=connection.prepareStatement(sql_1);
            preparedStatement.setString(1,id_leader);
            preparedStatement.executeUpdate();
