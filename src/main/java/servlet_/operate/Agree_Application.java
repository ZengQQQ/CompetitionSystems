...
public class Agree_Application extends HttpServlet {
    ...
    protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try{
            String sql="select Exist_Num from team_table where Student_ID_leader=?";
        ...
        //队长将申请者加入队伍
        try {
            String sql="insert into group_table (TID,UID,state) values (?,?,?)";
            connection=Druid_Utils.getConnection();
            preparedStatement=connection.prepareStatement(sql);
            preparedStatement.setString(1,leader_id);
        ...
        //设置队伍的空余位置-1
        try {
            String sql="update team_table set Exist_Num=Exist_Num-1 where Student_ID_leader=?";
            connection=Druid_Utils.getConnection();
            preparedStatement=connection.prepareStatement(sql);
            preparedStatement.setString(1,leader_id);
            preparedStatement.executeUpdate();
        ...
        //删除reply表中的申请信息
        try {
            String sql="delete from reply where send_id=? and receive_id=? and message=?";
            connection=Druid_Utils.getConnection();
            preparedStatement=connection.prepareStatement(sql);
        ...
        //队长向申请者发出通知
        try {
            String sql="insert into  reply (send_id,receive_id,message) values (?,?,?)";
            connection=Druid_Utils.getConnection();
            preparedStatement=connection.prepareStatement(sql);
            preparedStatement.setString(1,leader_id);
            preparedStatement.setString(2,id);
